...
import { setupSubcomponents } from './editor-subcomponents.modified.js';
...
const CFG = {
  ...
  // Target on-screen gizmo sizes (approx): 1.5cm ~ 58px, 2.0cm ~ 76px
  gizmoPxSub: 58,
  gizmoPxNormal: 76
};
...
function createTranslateGizmo() {
  ...
  gizmo.userData.baseSizeWorld = L; // used for screen-size scaling
...
function createRotateGizmo() {
  ...
  gizmo.userData.baseSizeWorld = r;
...
function createScaleGizmo() {
  ...
  gizmo.userData.baseSizeWorld = len;
  const s = 0.3;
...
function createGizmoFor(mode, position, targetObject, gizmoScale = 1.0) {
  ...
  // Screen-size scaling: keep gizmo ~constant on screen.
  const isSub = gizmoScale < 1.0;
  applyGizmoScreenScale(currentGizmo, isSub ? CFG.gizmoPxSub : CFG.gizmoPxNormal);
  scene.add(currentGizmo);
  return currentGizmo;
}


function applyGizmoScreenScale(gizmo, targetPx) {
  if (!gizmo || !camera) return;
  const base = gizmo.userData.baseSizeWorld || CFG.arrowSize || 2.0;

  // Distance from camera to gizmo
  const dist = camera.position.distanceTo(gizmo.position);
  const fov = THREE.MathUtils.degToRad(camera.fov);
  const worldPerScreenHeight = 2 * dist * Math.tan(fov * 0.5);
  const worldSize = worldPerScreenHeight * (targetPx / Math.max(1, window.innerHeight));

  const scl = worldSize / Math.max(1e-6, base);
  gizmo.scale.setScalar(scl);
}
function updateGizmoPose() {
  ...
  // keep target on-screen size
  const isSub = (currentMode === 'select');
  applyGizmoScreenScale(currentGizmo, isSub ? CFG.gizmoPxSub : CFG.gizmoPxNormal);
...
function refreshSubButtonsFromState() {
  ...
  if (btnSubMulti) setSubButtonActive(btnSubMulti, subAPI.getFlags().multi);
}
...
btnSubMulti?.addEventListener('click', () => {
  // Toggle single vs multi selection for subcomponents
  subAPI.setFlags({ multi: !subAPI.getFlags().multi });
  refreshSubButtonsFromState();
});
/* =============================
   INIT
