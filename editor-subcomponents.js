...
export function setupSubcomponents(api) {
  ...
  const state = {
    flags: { verts: true, edges: false, faces: false, explode: false, multi: false },
    selection: [], // [{kind:'v'|'e'|'f', key:string, indices:number[], centroidLocal:Vector3}]
    baseline: null // { id, positions: Float32Array copy }
  };

  // Radiant selection colors (categorical)
  const SEL_COLORS = {
    v: new THREE.Color(0xFF9500), // orange
    e: new THREE.Color(0x34C759), // green
    f: new THREE.Color(0x00BFFF)  // cyan
  };
  ...
  function ensureHelpers(obj) {
    ...
    if (!obj.userData.sub.selOverlay) {
      const g = new THREE.Group();
      g.name = 'SelectionOverlay';
      g.renderOrder = 999;
      g.visible = true;
      obj.add(g);
      obj.userData.sub.selOverlay = g;
    }
  ...
  function recolorSelection(obj) {
    ...
    if (obj.userData.sub.vertexPoints) {
      ...
      // mark selected vertex indices (orange)
      const selectedSet = new Set();
      state.selection.filter(s => s.kind === 'v').forEach(s => s.indices.forEach(i => selectedSet.add(i)));
      selectedSet.forEach(i => col.setXYZ(i, SEL_COLORS.v.r, SEL_COLORS.v.g, SEL_COLORS.v.b));

      col.needsUpdate = true;
    }

    // --- Edges/Faces overlay lines (categorical) ---
    const overlay = obj.userData.sub.selOverlay;
    if (!overlay) return;
    ...
    // Helper: push segment a->b
    function pushSeg(arr, ia, ib) {
      arr.push(pos.getX(ia), pos.getY(ia), pos.getZ(ia));
      arr.push(pos.getX(ib), pos.getY(ib), pos.getZ(ib));
    }
    ...
    for (const s of state.selection) {
      if (s.kind === 'e' && s.indices.length >= 2) {
        pushSeg(edgeVerts, s.indices[0], s.indices[1]);
      } else if (s.kind === 'f' && s.indices.length >= 3) {
        const [a,b,c] = s.indices;
        pushSeg(faceVerts, a, b);
        pushSeg(faceVerts, b, c);
        pushSeg(faceVerts, c, a);
      }
    }

    if (edgeVerts.length) {
      const g = new THREE.BufferGeometry();
      g.setAttribute('position', new THREE.BufferAttribute(new Float32Array(edgeVerts), 3));
    ...
    if (faceVerts.length) {
      const g = new THREE.BufferGeometry();
      g.setAttribute('position', new THREE.BufferAttribute(new Float32Array(faceVerts), 3));
      const m = new THREE.LineBasicMaterial({
        color: SEL_COLORS.f,
        transparent: true,
        opacity: 0.75,
  ...
  function togglePick(raycaster, obj) {
    ...
    if (state.flags.verts && obj.userData.sub.vertexPoints) {
      ...
      if (hits.length) {
        ...
        // Single-select mode: replace selection with this one
        if (!state.flags.multi && existing < 0) state.selection = [];
